...
public class Sudoku {
    public static void main(String[] args) {
        ...
        ArrayList<String> solutions = search(grid);
        if (!isValid(grid))
            System.out.println("Invalid input");
        else if(solutions.size() != 0) {
            System.out.println("\n" + solutions.size() + " Solution found \n");
    ...
    public static ArrayList<String> search(int[][] grid) {
        ...
        ArrayList<String> solutions = new ArrayList<>();  // Free cells
        if (freeCellList.length == 0) {
            solutions.add(printGrid(grid));
            return solutions; // "No free cells");
        }
        ...
        while (true) {
            ...
            if (isValid(i, j, grid)) {
                if (k + 1 == freeCellList.length) { // No more free cells
                    solutions.add(printGrid(grid));
                    return solutions; // A solution is found
